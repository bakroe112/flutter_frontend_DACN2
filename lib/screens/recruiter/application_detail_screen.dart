...
class _ApplicationDetailScreenState extends State<ApplicationDetailScreen> {
  ...
  void fetchApplicationDetail() async {
    ...
    if (result['success']) {
      final app = Application.fromJson(result['application'][0]);
      
      // Chỉ update status nếu là "Applied" và chưa được viewed
      if (app.status == "Applied") {
        await ApplicationService().updateStatusApplication(
          app.id,
          "Viewed",
          app.title,
          app.companyName!,
        );
        // Gọi lại để lấy data mới sau khi update
        fetchApplicationDetail();
        return;
      }
